...
var Container = React.createClass({
  ...
  handleClick: function () {
    ...
    // see if it matches the compare function 
      ...
      //add the animal to the div that the user clicked on 
  ...
  guess: function( id, e ) {
    ...
    var clickedDiv = $('#title'+id+' + div > .cat-wrapper > .cat-title .guessAnimal');
    console.log( clickedDiv )
    if ( clickedDiv.length === 0 ) {
      console.log('you failed!')
    }
    else {
      var guessAnimal = document.querySelector('.guessAnimal');

      if ( guessAnimal ) {
        guessAnimal.classList.remove('guessAnimal');
      }
    }
      ...
      return keys.map(function( key, i ){//took ou tthe i next to key            
        if ( !isNaN(parseInt( key ) ) ) {
          ...
          var key = node[ 0 ].map(function (i, idx) {
           ...
           var isShow = 'animal guessAnimal';
            return (
             <div id={"key"+i.AnimalName} className= {isShow} data-id={i.id}>
              {i.AnimalName}
             </div>
            );
          });
